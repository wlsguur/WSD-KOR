...
fname1 = 'MXLS2002107210.json' # v2.0에 추가된 메신저 대화 말뭉치 (오타, 비문 많음)
fname2 = 'NXLS2002104060.json'
fname3 = 'SXLS2002104060.json'
...
def process(sent_input, multiwords_set):
    ...
    nn_list = ['NNG', 'NNP'] # 일반명사, 고유명사인 WSD만 추출 (총 221,241개 중 127,011 + 6,922개)
    WSD_multiwords = []
    for wsd_d in sent_input2['WSD']:
        ...
        if wsd_d['word'] in multiwords_set and wsd_d['sense_id'] not in (777,888,999) and wsd_d['pos'] in nn_list:
            del wsd_d['pos']
            del wsd_d['begin']
            del wsd_d['end']
            del wsd_d['word_id']
...
if __name__ == "__main__":
    ...
    with open(os.path.join(data_path, fname3), "r") as json_file:
    ...
    # v2.0에 추가된 메신저 대화 포함 여부
    include_messenger = False
    if include_messenger:
        data = f1['document'] + f2['document'] + f3['document']
    else:
        data = f2['document'] + f3['document']
